...
<body>
  ...
  <div style="margin-top:40px;padding-top:16px;border-top:1px solid #e4e1da">
    <p style="font-size:10px;color:#9ca3af;line-height:1.65;margin-bottom:10px">
      This document is prepared for informational and planning purposes only. It does not constitute
      tax advice and should not be relied upon as such. All figures are estimates based on information
      provided and are subject to change. Consult a qualified tax advisor before making any financial
      decisions. ${firmName ? firmName + ' and its advisors assume' : 'The preparer assumes'} no
      liability for actions taken based on this summary.
    </p>
...
export function SalarySchedule({ client: c, onChange }: Props) {
  ...
  const prevEntry: SalaryScheduleEntry = c.salSched?.[activePerson]?.[prevYear] ?? { ...EMPTY_ENTRY }
  const prevCalc = useMemo(
    () => (compareMode ? calcSal(prevEntry, prevYear) : null),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [compareMode, prevYear, JSON.stringify(prevEntry)],
  )
  ...
  return (
    <div className="flex flex-col gap-3.5">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={cn(
              'flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[12px] font-semibold border transition-colors',
              compareMode ? 'bg-navy text-white border-navy' : 'bg-white text-text-lt border-border hover:border-navy/30',
            )}
          >
            ↔ vs {prevYear}
          </button>
      ...
      {compareMode && prevCalc && calc.gross > 0 && (
        <Card>
          <CardHeader>
            <CardTitle>Year-over-Year</CardTitle>
            <div className="flex gap-4 text-[11px] font-semibold">
              <span className="text-text-lt">{prevYear}</span>
              <span className="text-navy">→ {year}</span>
            </div>
          </CardHeader>
          <CardBody className="p-0">
            <table className="w-full text-[13px]">
              ...
              <tbody>
                {([
                  ['Gross salary',    prevCalc.gross,                          calc.gross],
                  ['401(k) deferral', prevCalc.tradDefer + prevCalc.rothDefer, calc.tradDefer + calc.rothDefer],
                  ['Federal WH',      prevCalc.fedWHTot,                       calc.fedWHTot],
                  ['SS tax',          prevCalc.ssTax,                          calc.ssTax],
                  ['Medicare',        prevCalc.medTax,                         calc.medTax],
                  ['State WH',        prevCalc.stateWHTot,                     calc.stateWHTot],
                  ['SDI',             prevCalc.sdiTax,                         calc.sdiTax],
                  ['Net take-home',   prevCalc.net,                            calc.net],
                ] as [string, number, number][]).map(([label, prev, curr]) => {
                  ...
                  return (
                    <tr key={label} className={cn('border-t border-border', isNet && 'bg-surface font-semibold')}>
                      ...
                      <td className={cn(
                        'px-4 py-2.5 text-right font-serif font-semibold',
                        delta > 0 ? 'text-success' : delta < 0 ? 'text-danger' : 'text-text-lt',
                      )}>
